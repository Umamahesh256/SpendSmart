...
export default function CreateMemberDueModal({ isOpen, onClose, groupId, month, year, members, memberProfiles, onSuccess }) {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      toast.success('Member Due created successfully!');
      onSuccess();
      onClose();
    } catch (err) {
      console.error(err);
      setError(err.message || 'Failed to create member due.');
    } finally {
    ...
    <div className="fixed inset-0 bg-black/60 backdrop-blur-sm z-50 flex items-center justify-center p-4">
      <div className="bg-surface border border-white/10 rounded-2xl w-full max-w-md overflow-hidden animate-scale-up">
        <div className="px-5 py-4 border-b border-white/5 flex justify-between items-center bg-white/[0.02]">
          <h2 className="text-lg font-bold">Create Member Due</h2>
        ...
        <form onSubmit={handleSubmit} className="p-5">
          ...
          <div className="mt-6">
            ...
            >
              <Save size={18} />
              {isSubmitting ? 'Creating...' : 'Create Member Due'}
            </button>
          </div>
        </form>
      </div>
